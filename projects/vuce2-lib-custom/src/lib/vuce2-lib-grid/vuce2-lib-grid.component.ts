...
import { Vuce2LibGridLocalDataSource } from "./vuce2-lib-grid.localDataSource";
import { Vuce2LibGridService } from "./vuce2-lib-grid.service";

@Component({
    selector: 'vuce2-lib-grid',
    templateUrl: './vuce2-lib-grid.component.html',
    styleUrls: ['./vuce2-lib-grid.component.scss'],
    providers: [Vuce2LibGridService]
})
export class Vuce2LibGridComponent implements OnChanges, OnDestroy, OnInit {
    ...
    @ContentChildren(Vuce2LibGridCellTemplateDirective) cellTemplates: QueryList<Vuce2LibGridCellTemplateDirective>;

    @Input()  borderHeader?: boolean | false;

    @Output() onRefresh: EventEmitter<boolean> = new EventEmitter<boolean>();

    $eventBus: Subscription;

    public constructor(private sanitizer: DomSanitizer, public state: Vuce2LibGridService) {
    ...
    public onChangeTable(): void {
        if (this.datasource) {
            ...
            if (this.datasource instanceof Array) {
                ...
                observable = new Vuce2LibGridLocalDataSource(this.datasource).asObservable(request);
            ...
            observable.subscribe(
                async (result: DatasourceResult) => {
                    ...
                    const newData = await this.setDetailTemplate(result);
    ...
    // L 291
    getCellTemplate(col: TableColumn, standardTemplate: TemplateRef<any>, detailTemplate: TemplateRef<any>): TemplateRef<any> {
        // //console.log(col);
        // //console.log(this.cellTemplates);
        if(col.name ==='expanded'){
            return detailTemplate;
        }
        let template = this.cellTemplates.filter(p => p.vuce2LibGridCellTemplate === col.name);
    ...
    getDetailTemplate():TemplateRef<any>{
        let template = this.cellTemplates.filter(c=>c.vuce2LibGridCellTemplate === 'detailTemplate');
        if (template.length>0){
            return template.map(p=> p.templateRef)[0];
        }
        return null;
    ...
    setDetailTemplate(result: DatasourceResult){
        return new Promise<DatasourceResult>(
            (resolve)=>{
                if(this.cellTemplates){
                    const template = this.cellTemplates.filter(c=>c.vuce2LibGridCellTemplate === 'detailTemplate');
                    if (template.length>0){
                        // Tiene detail
                ...
                const right = this.cellTemplates.filter(c=>c.vuce2LibGridCellTemplate === 'iconsRight');
                if(right.length>0)
                {
                    const originalIndex = this.columns.findIndex(x=>x.name=="iconsRight");
                    const finalIndex = this.columns.length -1;
                    this.columns = this.onMoveItemArray(this.columns, originalIndex, finalIndex);
                }

                const left = this.cellTemplates.filter(c=>c.vuce2LibGridCellTemplate === 'iconsLeft');
                if(left.length>0){
                    const originalIndex = this.columns.findIndex(x=>x.name=="iconsLeft");
                    const finalIndex = 0;
                    this.columns = this.onMoveItemArray(this.columns, originalIndex, finalIndex);
                }
                resolve(result);
    ...
    onMoveItemArray(arr:any, fromIndex:number, toIndex:number) {
